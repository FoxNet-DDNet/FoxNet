...
void CServerBan::ConBanClientId(IConsole::IResult *pResult, void *pUser)
{
	...
	if(UserId >= 0)
	{
		...
		char aTitle[32];
		str_format(aTitle, sizeof(aTitle), "[BAN] - Command (%d%s)", pThis->Server()->Port(), FormatServerInsntance(" | "));
...
void CServerBan::ConBanExt(IConsole::IResult *pResult, void *pUser)
{
	...
	if(str_isallnum(pStr))
	{
		...
		if(!pThis->Server()->ClientSupportsServerMaxClients(pResult->m_ClientId))
			log_info("net_ban", "ban error (use a more recent DDNet client)");
...
void CServer::SendMapData(int ClientId, int Chunk)
{
	...
	const unsigned int NumChunks = (MapSize + ChunkSize - 1) / ChunkSize;
...
void CServer::OnNetMsgReady(int ClientId)
{
	...
	// <FoxNet: m_OverrideMapActive is the equivalent of a rejoin for a per-client map
	if(m_aClients[ClientId].m_Rejoining || m_aClients[ClientId].m_OverrideMapActive)
	{
...
void CServer::UpdateDebugDummies(bool ForceDisconnect)
{
	...
	for(int DummyIndex = 0; DummyIndex < std::max(m_PreviousDebugDummies, g_Config.m_SvAddDummies); ++DummyIndex)
	{
...
void CServer::OverrideClientName(int ClientId, const char *pName)
{
	...
	// The snapped client info is cached per player, so it has to be told the name changed
	GameServer()->OnClientInfoChange(ClientId);
}
...
bool CServer::FoxNetNetMsg(int ClientId, int Msg, CUnpacker Unpacker)
{
	...
	case NETMSG_IAM_KAIZO:
	{
		SetCustomClient(ClientId, "Kaizo", Unpacker);
		return true;
	}
