...
int CNetServer::Recv(CNetChunk *pChunk, SECURITY_TOKEN *pResponseToken, int *pNumPacketsProcessed, int MaxPackets)
{
	while(true)
	{
		...
		else // connection-oriented packet
		{
			...
			// <FoxNet: the slot was online when GetClientSlot picked it, but a rejoin control message
			// re-enters the server above: it resends the map and runs the antibot join hooks, any of
			// which can drop the client. Feeding an offline connection asserts. FoxNet>
			if(Slot != -1 &&
				m_aSlots[Slot].m_Connection.State() != CNetConnection::EState::OFFLINE &&
