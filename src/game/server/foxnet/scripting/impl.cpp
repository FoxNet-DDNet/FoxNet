...
CScriptingCtx::CScriptingCtx()
{
	...
	m_pData->m_Chai.add_global(chaiscript::var(std::string()), "args");
}
...
void CScriptingCtx::Run(IStorage *pStorage, const char *pFilename, const char *pArgs)
{
	...
	try
	{
		// Update 'args' value instead of re-adding the symbol
		m_pData->m_Chai.set_global(chaiscript::var(std::string(pArgs ? pArgs : "")), "args");

		pScript = ReadScript(pStorage, pFilename);

		// Wrap the script into a new block scope so top-level `var` can be redeclared each run
		std::string Wrapped = "{\n";
		Wrapped += pScript;
		Wrapped += "\n}\n";
