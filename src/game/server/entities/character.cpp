...
CCharacter::CCharacter(CGameWorld *pWorld, int MapIdx, CNetObj_PlayerInput LastInput) :
	CEntity(pWorld, MapIdx, CGameWorld::ENTTYPE_CHARACTER, false, vec2(0, 0), CCharacterCore::PhysicalSize())
{
	m_Health = 0;
	m_Armor = 0;
...
void CCharacter::HandleJetpack()
{
	...
	case WEAPON_GUN:
	{
		if(m_Core.m_Jetpack)
		{
			float Strength = GetCurrentTuning()->m_JetpackStrength;
...
void CCharacter::HandleNinja()
{
	...
	if(m_Core.m_Ninja.m_CurrentMoveTime > 0)
	{
		...
		vec2 GroundElasticity = vec2(
			GetCurrentTuning()->m_GroundElasticityX,
			GetCurrentTuning()->m_GroundElasticityY);
...
void CCharacter::FireWeapon()
{
	...
	case WEAPON_HAMMER:
	{
		...
		for(int i = 0; i < Num; ++i)
		{
			...
			for(CServerComponent *pComponent : GameServer()->m_vpComponents)
				pComponent->OnCharacterHammerHit(m_pPlayer->GetCid(), pTarget->GetPlayer()->GetCid());
		...
		if(Hits != 0)
		{
			float FireDelay = GetCurrentTuning()->m_HammerHitFireDelay;
	...
	case WEAPON_SHOTGUN:
	{
		float LaserReach = GetCurrentTuning()->m_LaserReach;

		new CLaser(&GameServer()->m_World, MultiMapIdx(), m_Pos, Direction, LaserReach, m_pPlayer->GetCid(), WEAPON_SHOTGUN);
	...
	{
		// <FoxNet
		if(GetCurrentTuning()->m_TeleGrenade) // KoG Compatibility
		{
		...
		int Lifetime = (int)(Server()->TickSpeed() * GetCurrentTuning()->m_GrenadeLifetime);
	...
	case WEAPON_LASER:
	{
		...
		new CLaser(GameWorld(), MultiMapIdx(), m_Pos, Direction, LaserReach, m_pPlayer->GetCid(), WEAPON_LASER);
...
float CCharacter::GetFireDelay(int Weapon)
{
	...
	case WEAPON_HAMMER: return (float)GetCurrentTuning()->m_HammerFireDelay;
	case WEAPON_GUN: return (float)GetCurrentTuning()->m_GunFireDelay;
	case WEAPON_SHOTGUN: return (float)GetCurrentTuning()->m_ShotgunFireDelay;
...
void CCharacter::SnapCharacter(int SnappingClient, int MapId)
{
	...
	if(!Server()->IsSixup(SnappingClient))
	{
		...
		if(m_pPlayer->GetCid() != SnappingClient || GetPlayer()->IsPaused())
		{
			if(g_Config.m_SvTeeCursor)
			{
				Character.m_VelY = 0;
			...
			{
				Character.m_X = round_to_int(GetSpecialPos().x);
				Character.m_Y = round_to_int(GetSpecialPos().y);
			}
		}
		// FoxNet>
...
bool CCharacter::CanSnapCharacter(int SnappingClient)
{
	...
	{
		if(pSnapPlayer->SpectatorId() != SPEC_FREEVIEW && !CanCollide(pSnapPlayer->SpectatorId()) && (pSnapPlayer->GetShowOthers() == SHOW_OTHERS_OFF || (pSnapPlayer->GetShowOthers() == SHOW_OTHERS_ONLY_TEAM && !SameTeam(pSnapPlayer->SpectatorId()))))
			return false;
		else if(pSnapPlayer->SpectatorId() == SPEC_FREEVIEW && !CanCollide(SnappingClient) && pSnapPlayer->m_SpecTeam && !SameTeam(SnappingClient))
			return false;
	}
	else if(pSnapChar && !pSnapChar->m_Core.m_Super && !CanCollide(SnappingClient) && (pSnapPlayer->GetShowOthers() == SHOW_OTHERS_OFF || (pSnapPlayer->GetShowOthers() == SHOW_OTHERS_ONLY_TEAM && !SameTeam(SnappingClient))))
		return false;
...
void CCharacter::Snap(int SnappingClient)
{
	...
	for(CServerComponent *pComponent : GameServer()->m_vpComponents)
	{
		if(!pComponent->CanSnapCharacter(this, SnappingClient))
			return;
	}
	...
	if(m_Core.m_CollisionDisabled || !GetCurrentTuning()->m_PlayerCollision)
		DDNetCharacter.m_Flags |= CHARACTERFLAG_COLLISION_DISABLED;
	if(m_Core.m_HookHitDisabled || !GetCurrentTuning()->m_PlayerHooking)
	...
	if(SnapPlayer)
	{
		...
		if(SnappingClient == m_pPlayer->GetCid() || SnapPlayer->Acc()->m_Configs.m_Cosmetics.m_ShowEffects)
		{
	...
	for(int Weapon = WEAPON_HAMMER; Weapon < NUM_EXTRA_WEAPONS; Weapon++)
		if(GetWeaponGot(Weapon))
			DDNetCharacter.m_Flags |= WeaponFlag(GameServer()->GetWeaponType(Weapon));

	if(pSnapChar)
	{
		if(m_InSnake || m_Ufo.Active())
		{
			DDNetCharacter.m_Jumps = 0;
			DDNetCharacter.m_JumpedTotal = 0;
			if(m_InSnake)
			{
		...
		if(pSnapChar->Core()->m_Passive)
			DDNetCharacter.m_Flags |= CHARACTERFLAG_HOOK_HIT_DISABLED | CHARACTERFLAG_HAMMER_HIT_DISABLED;
		if(!pSnapChar->Core()->m_Hittable && m_pPlayer->GetCid() != SnappingClient)
			DDNetCharacter.m_Flags |= CHARACTERFLAG_HAMMER_HIT_DISABLED;
		if(!pSnapChar->Core()->m_Hookable && m_pPlayer->GetCid() != SnappingClient)
			DDNetCharacter.m_Flags |= CHARACTERFLAG_HOOK_HIT_DISABLED;
	}
	...
	if(!m_Core.m_Collidable && m_pPlayer->GetCid() == SnappingClient)
		DDNetCharacter.m_Flags |= CHARACTERFLAG_COLLISION_DISABLED;
	if(!GetCurrentTuning()->m_PlayerHammering)
		DDNetCharacter.m_Flags |= CHARACTERFLAG_HAMMER_HIT_DISABLED;
	// FoxNet>
...
void CCharacter::HandleSkippableTiles(int Index)
{
	...
	if(Collision()->IsSpeedup(Index))
	{
		...
		else if(Type == TILE_SPEED_BOOST)
		{
			...
			if(MaxSpeed == 0)
			{
				float MaxRampSpeed = GetCurrentTuning()->m_VelrampRange / (50 * log(std::max((float)GetCurrentTuning()->m_VelrampCurvature, 1.01f)));
				MaxSpeed = std::max(MaxRampSpeed, GetCurrentTuning()->m_VelrampStart / 50) * MaxSpeedScale;
			}
...
bool CCharacter::FreezeForce(int Ticks)
{
	m_Armor = 0;
	m_FreezeTime = Ticks;
...
bool CCharacter::Freeze(int Seconds)
{
	...
	if(m_FreezeTime == 0 || m_Core.m_FreezeStart < Server()->Tick() - Server()->TickSpeed())
	{
		FreezeForce(Seconds * Server()->TickSpeed());
		return true;
	}
	return false;
}
...
{
	for(CServerComponent *pComponent : GameServer()->m_vpComponents)
		pComponent->OnCharacterDie(GetPlayer()->GetCid(), Killer, Weapon, SendKillMsg);
...
void CCharacter::FoxNetTick()
{
	...
	if(GetPlayer()->m_SpiderHook)
	{
		...
		float HookLength = abs(GetCurrentTuning()->m_HookLength - 20.0f);
...
{
	int Zone = std::clamp(pZone, -1, TuneZone::NUM - 1);
	if(Zone == 0)
		Zone = -1;
...
void CCharacter::DropWeapon(int Type, vec2 Dir, bool Death)
{
	...
	for(CServerComponent *pComponent : GameServer()->m_vpComponents)
	{
		if(!pComponent->CanDropWeapon(this, Type))
			return;
	}
...
{
	vec2 tmp;
	return Collision()->IntersectLine(m_Pos, Pos, &tmp, &tmp) == 0;
}
