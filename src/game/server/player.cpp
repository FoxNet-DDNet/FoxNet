...
void CPlayer::Tick()
{
	...
	int CurrentIndex = GameServer()->Collision(MultiMapIdx())->GetMapIndex(m_ViewPos);
	if(m_pCharacter)
		m_TuneZone = m_pCharacter->GetOverriddenTuneZone();
	else
		m_TuneZone = GameServer()->Collision(MultiMapIdx())->IsTune(CurrentIndex);
...
void CPlayer::Snap(int SnappingClient)
{
	...
	// None of this depends on SnappingClient, so it is packed once and rebuilt only when the
	// name, clan, country or tee infos change. FoxNet's per-viewer overrides run on a copy.
	...
	CNetObj_ClientInfo ClientInfo = m_ClientInfo;
	...
	if(!Server()->IsSixup(SnappingClient))
	{
		...
		if(SnappingClientVersion < VERSION_DDNET_INDEPENDENT_SPECTATORS_TEAM)
		{
			...
			PlayerInfo.m_Team = (m_Paused != PAUSE_PAUSED || m_ClientId != SnappingClient) && m_Paused < PAUSE_SPEC ? Team : TEAM_SPECTATORS;
		}
	...
	if(m_ClientId == SnappingClient)
	{
		...
		const int SpectatingClient = ((Team != TEAM_SPECTATORS && !m_Paused) || m_SpectatorId < 0 || m_SpectatorId >= Server()->MaxClients()) ? m_ClientId : m_SpectatorId;
		...
		if(pSpecPlayer)
		{
			...
			if(pSpecPlayer->m_EnableSpectatorCount && SpectatingClient == m_ClientId && SnappingClient != SERVER_DEMO_CLIENT && Team != TEAM_SPECTATORS && !m_Paused)
			{
				...
				for(auto &pPlayer : GameServer()->m_apPlayers)
				{
					...
					if(!Server()->ClientIngame(SpecClientId) || !pPlayer->m_EnableSpectatorCount || SpecClientId == m_ClientId || pPlayer->m_Afk ||
...
{
	m_PrevPlayerFlags = m_PlayerFlags;
...
{
	m_Spawning = false;
	m_pCharacter = new(m_ClientId) CCharacter(&GameServer()->m_World, MultiMapIdx(), GameServer()->GetLastPlayerInput(m_ClientId));
...
void CPlayer::TryRespawn()
{
	...
	GameServer()->CreatePlayerSpawn(SpawnPos, GameServer()->m_pController->GetMaskForPlayerWorldEvent(m_ClientId, MultiMapIdx()));
...
void CPlayer::ProcessPause()
{
	...
	if(m_Paused == PAUSE_SPEC && !m_pCharacter->IsPaused() && CanSpec())
	{
		...
		GameServer()->CreateDeath(m_pCharacter->m_Pos, m_ClientId, GameServer()->m_pController->GetMaskForPlayerWorldEvent(m_ClientId, MultiMapIdx()));
		GameServer()->CreateSound(m_pCharacter->m_Pos, SOUND_PLAYER_DIE, GameServer()->m_pController->GetMaskForPlayerWorldEvent(m_ClientId, MultiMapIdx()));
	}
}
	...
	if(State != m_Paused)
	{
		...
		case PAUSE_NONE:
			if(m_pCharacter->IsPaused()) // First condition might be unnecessary
			{
				...
				m_ViewPos = m_pCharacter->m_Pos;
				GameServer()->CreatePlayerSpawn(m_pCharacter->m_Pos, GameServer()->m_pController->GetMaskForPlayerWorldEvent(m_ClientId, MultiMapIdx()));
			}
			[[fallthrough]];
		case PAUSE_SPEC:
...
void CPlayer::ProcessScoreResult(CScorePlayerResult &Result)
{
	if(Result.m_Success) // SQL request was successful
	{
		...
		case CScorePlayerResult::PLAYER_INFO:
		{
			...
			// <FoxNet
			if(Birthday != 0 && !m_BirthdayAnnounced && GetCharacter() && !Server()->QuietJoin(m_ClientId))
			{
				char aBuf[512];
				str_format(aBuf, sizeof(aBuf),
					"Happy FoxNet birthday to %s for finishing their first map %d year%s ago!",
					Server()->ClientName(m_ClientId), Birthday, Birthday > 1 ? "s" : "");
				GameServer()->SendChat(-1, TEAM_ALL, aBuf, m_ClientId);
				str_format(aBuf, sizeof(aBuf),
					"Happy FoxNet birthday, %s!\nYou have finished your first map exactly %d year%s ago!",
					Server()->ClientName(m_ClientId), Birthday, Birthday > 1 ? "s" : "");
