...
void CPlayer::Tick()
{
	...
	int CurrentIndex = GameServer()->Collision(m_ClientId)->GetMapIndex(m_ViewPos);
	if(m_pCharacter)
		m_TuneZone = m_pCharacter->GetOverriddenTuneZone();
	else
		m_TuneZone = GameServer()->Collision(m_ClientId)->IsTune(CurrentIndex);
...
{
	m_Spawning = false;
	m_pCharacter = new(m_ClientId) CCharacter(&GameServer()->m_World, GameServer()->Collision(m_ClientId), GameServer()->GetLastPlayerInput(m_ClientId));
