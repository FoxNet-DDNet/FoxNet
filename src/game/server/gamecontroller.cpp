			...
			if(j == 0)
			{
				...
				for(int Index = 0; Index < 5 && Result == -1; ++Index)
				{
					...
					for(int c = 0; c < Num; ++c)
					{
						...
						if(GameServer()->Collision(pChr->GetCore().m_MultiMapIdx)->CheckPoint(SpawnPoint + aPositions[Index]) ||
		...
		else
		{
			for(CPlayer *pRec : GameServer()->m_apPlayers)
			{
				...
				if(Server()->ClientSlotEmpty(pRec->GetCid()))
					continue;
				if(pRec->m_Vanish || Server()->QuietJoin(pRec->GetCid()) || Server()->IsRconAuthed(pRec->GetCid()))
					pRec->SendChat(aBuf);
			}
		}
		// FoxNet>
...
void IGameController::Snap(int SnappingClient)
{
	...
	GameInfoEx.m_Flags =
		...
		GAMEINFOFLAG_RACE;
	// GameInfoEx.m_Flags2 = GAMEINFOFLAG2_HUD_DDRACE | GAMEINFOFLAG2_DDRACE_TEAM | GAMEINFOFLAG2_PREDICT_EVENTS | GAMEINFOFLAG2_SUPPORTS_128_TEAMS;
	GameInfoEx.m_Flags2 = GAMEINFOFLAG2_HUD_DDRACE | GAMEINFOFLAG2_DDRACE_TEAM | GAMEINFOFLAG2_SUPPORTS_128_TEAMS;
	...
	if(g_Config.m_SvAllowZoom)
		GameInfoEx.m_Flags |= GAMEINFOFLAG_ALLOW_ZOOM;
	if(g_Config.m_SvAllowHookColl)
		GameInfoEx.m_Flags |= GAMEINFOFLAG_ALLOW_HOOK_COLL;
	if(g_Config.m_SvAllowEyeWheel)
		GameInfoEx.m_Flags |= GAMEINFOFLAG_ALLOW_EYE_WHEEL;
	GameInfoEx.m_Flags2 |= GAMEINFOFLAG2_ALLOW_X_SKINS;

	if(g_Config.m_SvMovingTilesStopTime)
		m_QuadStartTick = 0x7FFFFFFF;
	else
		m_QuadStartTick = m_RoundStartTick;
	GameInfo.m_RoundStartTick = m_QuadStartTick;
	...
	if(Server()->IsSixup(SnappingClient))
	{
		...
		GameData.m_GameStartTick = m_QuadStartTick;
